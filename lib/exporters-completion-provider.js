...
const getQuoteChar = require('./get-quote-char')
...
class ExportersCompletionItemProvider {
  provideCompletionItems (document, position, token) {
    ...
    const quoteChar = config.quoteCharOverride || getQuoteChar(thisDocumentFileName)
    ...
    const addImportStatementToCompletionItem = (ci, ex, importSource, importToken) => {
      ...
      if (existingImportNode) {
        ...
        ci.additionalTextEdits = [
          TextEdit.insert(positionForNewImport, `import ${importToken} from ${quoteChar}${importSource}${quoteChar}${maybeSemi}\n`)
        ]
      }
      ...
      if (thisDocumentFileName !== fileName) {
        fileExports.forEach((ex) => {
          ...
          if (line.text.startsWith('import')) {
            const importStatement = `import ${importToken} from ${quoteChar}${relPath}${quoteChar}${maybeSemi}\n`
    ...
    exportsInProject.dependencies.forEach((dependencyExports, depName) => {
      dependencyExports.forEach((ex) => {
        ...
        if (line.text.startsWith('import')) {
          const importStatement = `import ${importToken} from ${quoteChar}${depName}${quoteChar}${maybeSemi}\n`
          ci.textEdit = TextEdit.replace(line.range, importStatement)
