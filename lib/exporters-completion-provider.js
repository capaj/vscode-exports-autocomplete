...
const {getDeclarations, getImportedTokens} = require('./get-imports')
const {babylonToVsCode} = require('./position-utils')
...
class ExportersCompletionItemProvider {
  provideCompletionItems (document, position, token) {
    ...
    const indentation = editorText.includes('\t') ? '\t' : '  '
    ...
    const addImportStatementToCompletionItem = (ci, ex, importSource, importToken) => {
      ...
      if (existingImportNode) {
        ...
        ci.additionalTextEdits = [
          TextEdit.insert(positionForNewImport, `import ${importToken} from '${importSource}'${maybeSemi}\n`)
        ]
      }
      ...
      if (thisDocumentFileName !== fileName) {
        fileExports.forEach((ex) => {
          if (importedTokens.includes(ex.name)) {
            return  // TODO instead of this, make a list o imported tokens and check in there
          }
